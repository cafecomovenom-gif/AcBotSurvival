...
-- Espera o LocalPlayer e PlayerGui (mais robusto para compatibilidade com executores como Delta/Fluxus)
local LocalPlayer
repeat
...
-- Parent fallback: prefer PlayerGui, if algo ainda der errado usar CoreGui (exploit permissões)
        ...
        pcall(function()
            ...
            if remotes then
                ...
                if damageRemote and damageRemote:IsA("RemoteEvent") then
                    damageRemote:FireServer(model, damage)
                    success = true
...
local function createSlider(labelText, min, max, step, parent, initialValue, onChange)
    ...
    local function updateValue(input)
        ...
        local value = math.floor(min + (relativeX * (max - min)) / step + 0.5) * step
...
local MobSection = createSection and createSection("Mob Aura Kill", "Automatically kill mobs within range", UDim2.new(0, 12, 0, 52), 1) or (function()
    -- fallback if createSection local isn't defined yet (shouldn't happen)
    return createSection("Mob Aura Kill", "Automatically kill mobs within range", UDim2.new(0, 12, 0, 52), 1)
end)()

-- Because we defined createSection earlier in the file, ensure sections are created correctly:
