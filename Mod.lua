...
--  Version: 2.1.0 (OPTIMIZED) - CORRIGIDO
--  Correções principais:
--   - Tornado robusto a execução em ambientes onde Players.LocalPlayer pode inicialmente ser nil
--   - Uso de :WaitForChild("PlayerGui") para garantir PlayerGui disponível
--   - Pequenas correções de segurança no loop de arrastar e nos eventos de Input para evitar erros
-- ═══════════════════════════════════════════════════════════
...
-- Espera o LocalPlayer e PlayerGui (robusto para diferentes ambientes)
...
if not LocalPlayer then
    -- tenta aguardar até que LocalPlayer exista (para LocalScript)
    local timeout = 10 -- segundos de timeout para não travar indefinidamente
    local start = tick()
    repeat
        task.wait(0.05)
        LocalPlayer = Players.LocalPlayer
        if LocalPlayer then break end
        -- fallback: pega primeiro jogador na lista (apenas se houver)
    ...
    if not LocalPlayer then
        warn("[Mirage] Não foi possível localizar LocalPlayer após espera. GUI pode não aparecer.")
    end
end

-- Se LocalPlayer encontrou, esperamos ou pegamos PlayerGui de forma robusta
local PlayerGui = nil
if LocalPlayer then
    -- Usa WaitForChild para garantir que PlayerGui exista quando LocalPlayer for válido
    PlayerGui = LocalPlayer:FindFirstChild("PlayerGui") or LocalPlayer:WaitForChild("PlayerGui", 5)
    -- Se ainda nil, tentamos acessar serviço CoreGui apenas como último recurso (pode falhar por permissões)
    if not PlayerGui then
        PlayerGui = game:GetService("CoreGui")
        warn("[Mirage] PlayerGui não encontrado, usando CoreGui como fallback (pode não funcionar dependendo do ambiente).")
    end
else
    -- Sem LocalPlayer encontrado, usamos CoreGui como último recurso
    PlayerGui = game:GetService("CoreGui")
end
...
local function createInstance(className, properties)
    ...
    for property, value in pairs(properties) do
        -- protege contra propriedades inexistentes que poderiam causar erro
...
-- Parent fallback: prefer PlayerGui; se nil, CoreGui já foi escolhido acima
...
local pulseConnection = RunService.RenderStepped:Connect(function()
    local time = tick()
    -- uso de math.clamp evita possível overflow no color components
...
local lastDamageTimes = setmetatable({}, {__mode = "k"}) -- weak keys to avoid memory leak
...
-- Função ROBUSTA para aplicar dano
local function applyDamage(model, damage)
    if not model or not model.Parent then return false end
    
    local success = false
    
    -- Método 1: Usar Humanoid:TakeDamage()
    ...
    -- Método 2: Modificar Health diretamente
        ...
        pcall(function()
            ...
            if remotes then
                ...
                if damageRemote and (damageRemote:IsA("RemoteEvent") or damageRemote:IsA("RemoteFunction")) then
                    -- se for RemoteFunction tenta chamar, se for RemoteEvent usa FireServer
    ...
    -- Método 5: Destruir partes (último recurso - pode causar problemas)
    if not success then
...
TitleBar.InputBegan:Connect(function(input)
    if input.UserInputType == Enum.UserInputType.MouseButton1 then
        ...
        -- captura o input para evitar a UI perder evento
        input:Capture()
    end
end)
